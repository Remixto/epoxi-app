...
import io.epoxi.engine.etl.pipeline.EtlPipeline;
import io.epoxi.engine.etl.pipeline.PipelineEndpoint;
import io.epoxi.engine.etl.pipeline.PipelineException;
...
import io.epoxi.repository.MessageQueueRepository;
import io.epoxi.repository.StepEndpointTemplateRepository;
import io.epoxi.repository.event.Event;
import io.epoxi.repository.event.EventStream;
...
private void logStepResult(Entry<QueueType, List<Event>> entry, TopicName topicName)
{
  ...
  try
  {
    ...
    writer.write(writeables.toArray(new Writable[0]));
  }
